...
function App() {
  ...
  return (
    ...
    <div>
      {data ? data.map(({node, parents, children}, index) => (
        <div key={index}>
          
            <div align="left">
              {parents.map(({IRI}, index) => (
                <div key={index}>
                  <h2>Parent IRI: {IRI}</h2>
                </div>
              ))}
            </div>
            ...
            <div align="right">
              {children.map(({IRI}, index) => (
                <div key={index}>
                  <h2>Children IRI: {IRI}</h2>
                </div>
              ))}
            </div>
        <hr/> 
  ...
  //     {data ? data.map(({node, parents, children}) => (
  //       <div align="left">
  //       {parents.map(({__typename , IRI, _updated, description,  isDefinedBy, label, level}) => (
